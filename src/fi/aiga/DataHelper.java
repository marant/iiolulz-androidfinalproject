...
public class DataHelper {
   ...
   public long insertLog(fi.aiga.Log log) {	   
   ...
   public List<Note> selectNotes(String cacheId) {
	...
	public List<fi.aiga.Log> selectLogs(String cacheId) {
	      List<fi.aiga.Log> list = new ArrayList<fi.aiga.Log>();
	      Cursor cursor = this.db.query(TABLE_NAME_LOGS, new String[] { "type","title","noteText","date","cacheId" },
	    		  "cacheId like '"+cacheId+"'", null, null, null, "date desc");
	      if (cursor.moveToFirst()) {
	         do {
	        	 fi.aiga.Log log = new fi.aiga.Log();
	...
	public List<String> selectFoundCacheIds() {
	      ...
	      if (cursor.moveToFirst()) {
	    	  do {
				 if(Integer.parseInt(cursor.getString(0)) == fi.aiga.Log.TYPE_FOUND)
